...
class HariomBhatt:
    name       = "Hariom Bhatt"
    role       = "Cybersecurity Professional | B.Tech CSE 2026"
...
### 🔧 Project 1 — DevSecOps Security Pipeline (CI/CD + Shift Left)
...
#### 🎯 What I Did
- ⚙️ Developed a **DevSecOps pipeline** using **GitHub Actions** to automate security checks across the SDLC
- 🔍 Implemented **Shift Left security** using **SAST (CodeQL)**, **SCA (Snyk)**, and **DAST (OWASP ZAP)** to detect vulnerabilities early
- 🚫 Enforced secure build policies to automatically block deployment of code with critical vulnerabilities
- 📊 Automated vulnerability detection, surfacing findings to developers before merging to production
...
| Area | Achievement |
|------|-------------|
| SAST | Automated static analysis with CodeQL on every PR |
| SCA | Dependency vulnerability scanning via Snyk |
| DAST | Dynamic runtime testing with OWASP ZAP |
| Enforcement | Blocked insecure builds via branch protection policies |
...
### 🔴 Project 2 — Web Application Penetration Testing – OWASP Top 10 on DVWA
...
Reconnaissance ──► Enumeration ──► Exploitation ──► Post-Exploitation ──► Reporting
     │                  │               │                   │                  │
  Target info      Dir busting     SQLi, CMDi,         Privilege          CVSS Scores
  Tech stack       Form mapping    Auth bypass,         escalation         PoC + Remediation
                                   XSS, SSRF
```

---

### 🟣 Project 3 — Network Vulnerability Assessment – Nmap & Metasploit
...
#### 🔬 Assessment Workflow
```
1. Host Discovery      →  nmap -sn <network_range>
2. Port Scanning       →  nmap -sV -sC -p- <target>
3. Service Enum        →  nmap --script vuln <target>
4. OS Fingerprinting   →  nmap -O <target>
5. Exploitation        →  msfconsole → search CVE-2011-2523 → exploit
6. Manual Exploitation →  Netcat for service interaction
7. Traffic Analysis    →  Wireshark packet capture
8. Risk Scoring        →  CVSS v3.1 Base Score Calculation
9. Reporting           →  Remediation-prioritized report (PTES-aligned)
```
